...
def make_callback(name, the_vqvae):
...
@click.option('--vqvae-checkpoint-path', default=None)
@click.option('--checkpoint-path', default=None)
@click.option('--config-path', default='')
@torch.no_grad()
def train_gpt(name, checkpoint_path, config_path, vqvae_checkpoint_path):
    ...
    model, opt_data, base_step = make_gpt(checkpoint_path, gpt_config)
    print(base_step)

    the_vqvae = load_vqvae(config.model.vqvae.params, vqvae_checkpoint_path)
    ...
    trainer = Trainer(model, train_dataset, test_dataset, config=TrainerConfig(
        ...
        max_epochs=1),
        callback=make_callback(name, the_vqvae))
